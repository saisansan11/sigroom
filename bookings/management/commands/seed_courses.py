...
class Command(BaseCommand):
    ...
    def _resolve_supervisor(self, username):
        ...
        candidates = [
            user for user in User.objects.filter(is_active=True).order_by("pk")
            if can_create_cohort(user)
        ]
        if len(candidates) != 1:
            raise CommandError(
                "กรุณาระบุ --default-supervisor <username> เมื่อผู้ใช้ที่มีสิทธิ์สร้างรอบมีไม่เท่ากับ 1 คน"
            )
        return candidates[0]
