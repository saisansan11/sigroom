...
- **Time Presets ในหน้าแก้ไขการจอง (Booking Edit) — แก้ไขใน UX-19:**
  - Draft edit ส่ง `time_presets` เข้า template context แล้ว จึงใช้ปุ่มช่วงเวลาสำเร็จรูปร่วมกับฟอร์มจองปกติได้โดยไม่สร้าง component ซ้ำ
  - หลังส่งคำขอ (Post-submit) ฟิลด์วันและเวลายังคงถูกล็อกตามนโยบาย `POST_SUBMIT_EDITABLE_FIELDS` เพื่อป้องกันการจองทับซ้อน และจะไม่ render ปุ่ม presets
  - สำหรับการจองที่ได้รับอนุมัติแล้วและต้องการปรับเปลี่ยนเวลา ยังคงใช้กระบวนการยื่นคำขอแก้ไข (Amendment) ตามขั้นตอน ไม่ใช่การยกเลิกแล้วจองใหม่ (Rebook)
